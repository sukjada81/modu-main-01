...
;(function ( $, window, document, undefined ) {
	...
	*/
	_setEvents: function() {
		...
		$(document).on("click", "html", function(e){ self._closeheap(true,function(){},function(){});});
	},
	...
	_setSliderEvents: function() {
		...
		heapTitle = $("#heapbox_"+this.instance.heapId);				

		if(this.options.twidth==1) heapTitle.css({border:this.options.tborder,"z-index":10});
		else {
			if(this.options.width.indexOf('%')!=-1) heapTitle.css({border:this.options.tborder,"z-index":this.options.zindex,'background-color':this.options.bgcolor, width:'calc(' + this.options.width + ' - 29px)'}); 
			else heapTitle.css({border:this.options.tborder,"z-index":this.options.zindex,'background-color':this.options.bgcolor, width:parseInt(this.options.width) - 28 + 'px'}); 
		}
		
		if(this.options.width==0) this.options.width = parseInt(heapTitle.css('width')) + 28 + 'px';

	    heap = $("#heapbox_"+this.instance.heapId+" .heap");		

		if(this.searchUsed) var top = parseInt(this.options.top) * 2 +'px';
		else var top = this.options.top;

		if(this.options.width.indexOf('%')!=-1) heap.css({width:'100%',top:top,left:this.options.left,'background-color':this.options.bgcolor, border:this.options.border});
		else heap.css({width:this.options.width,top:top,left:this.options.left,'background-color':this.options.bgcolor, border:this.options.border});
	...
	_setViewPosition: function(heapbox) {		
		$("div#heapbox_"+this.instance.heapId).css({cursor:'pointer'});
		heap = $("div#heapbox_"+this.instance.heapId+" .heap");		
		heap.show();

		heapbox.find(".heapOptions li a").css({'text-indent':this.options.tabIndent});
	...
	_setKeyboardEvents: function() {
		...
		heapbox.keydown(function(e) {
			...
			{
				/*
				case 13: self._handlerClicked('', 1);
						 return false;
						 break;
				*/
	...
	_keyArrowHandler:function(heapboxEl,direction){
		...
		heapboxEl.find("div.heap ul li").each(function(){			
			...
			{	
				...
				if(selectItem) {
					self._heapChanged(self,selectItem,true,direction);
					return false;
				}
			}
		});
	...
	*/
    _createElements: function() {		
		...
		heapBoxEl.append(heapBoxSearch);
		heapBoxEl.append(heapBoxheapEl);
		this.heapBoxEl = heapBoxEl;
		this._insertHeapbox(this.heapBoxEl);		

		$("div#heapbox_"+this.instance.heapId+" .handler").html('<i class="xi-angle-down"></i>');
    },
	...
	*/
    ...
    _setHeapSize: function() {
		...
		if(!this.options.heapsize) {
			this.options.heapsize = $("#heapbox_"+this.instance.heapId).attr("heapsize");
		}
		if(this.options.heapsize) {			
			$("#heapbox_"+this.instance.heapId).attr("heapsize", this.options.heapsize);
			if($("div#heapbox_"+this.instance.heapId+" .heapSearch").length) {
				$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('input:text').val('');
				$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('.heapResetIcon').hide();		
				$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('.heapSearchIcon').show();
			}

			if(heapBoxheapEl.height() == parseInt(this.options.heapsize)) {
				tmp_height	= heapBoxheapEl.find(".heapOptions .heapOption").css("height");
				tmp_cnt		= heapBoxheapEl.find(".heapOptions .heapOption").length
				ckHeight	= parseInt(tmp_height) * parseInt(tmp_cnt);				
			}
			else ckHeight	= heapBoxheapEl.height();
			
			if(ckHeight < parseInt(this.options.heapsize,10)) {
				if(this.options.searchUsed) this.searchUsed = false;
				heapBoxheapEl.css({"height": "auto"});
				delete this.options.heapsize;
				return;
			} else {
				if(this.options.searchUsed) this.searchUsed = true;
				heapBoxheapEl.css({"height":this.options.heapsize, "overflow":"auto"});
			}		
		}
    },
    ...
    _setHolderTitle: function()
    {
		...
		if(this.options.bgcolor){
			holderEl.css("background", this.options.bgcolor);
			$("#heapbox_"+this.instance.heapId).find(".handler").css("background", this.options.bgcolor);
		}
    ...
    _setData: function(data)
    {
		...
		if(this.searchUsed && this.searchOk == 'N') this._setSearch();
	...
	/*
		Set Search by 모두복지몰.kr
	*/
	_setSearch: function() {

		if(this.searchYN=='Y') return;

		var self = this;
			
		heapSearch = $("div#heapbox_"+this.instance.heapId+" .heapSearch");

		if(this.options.width.indexOf('%') != -1) heapSearch.css({width:'calc(100% + 9px)', top:parseInt(this.options.top) - 1 + 'px'});
		else heapSearch.css({width:parseInt(this.options.width) + 10 +'px', top:parseInt(this.options.top) - 1 + 'px'});	

		var searchField =	$("<input type='text' class='heap-search' style='padding:6px 12px 5px 12px; width:calc(100% - 33px);' placeholder='검색어 + 엔터키'><p class='heapSearchIcon'><i class='xi-search colorGray masterTooltip' title='검색'></i></p><p class='heapResetIcon'><i class='xi-close colorGray masterTooltip' title='취소'></i></p>",{
								style:"width:100%;"
							}).unbind('keyup').bind("keyup",function(e){
								if($(this).val() == '') {
									self._searchResetHeap();
									return;
								}	
								...
								if (e.keyCode == 13) {
									...
									if($(this).val()) {			
										$(this).siblings('.heapSearchIcon').hide();
										$(this).siblings('.heapResetIcon').show();
										self._searchHeap($(this).val());
									}									 
								}
							});					

		$(searchField).appendTo(heapSearch);		
		
		heapSearch.find('.heapSearchIcon').click(function(){					
			$(this).hide();			
			$(this).siblings('.heapResetIcon').show();
			self._searchHeap($(this).siblings('.heap-search').val());
		});

		heapSearch.find('.heapResetIcon').click(function(){
			$(this).siblings('.heap-search').val('');
			self._searchResetHeap();				
		});
		
		heapSearch.hover(function(e){
		...
		this.tmpHeapSize = this.options.heapsize;
		this.searchOk	= 'Y';
	...
	*/
	_handlerClicked: function(stageReady, ck) {
		
		if(this.instance.state)
		{	
			if(ck==1) this.searchClick = false;
	        this._closeheap();
		}
		else
		{
	...
	*/
	_heapChanged: function(self,clickedEl,keepOpened,direction) {
		...
		if(direction) {
			...
			firstTop	= heapbox.find(".heapOptions li a").first().offset().top;
			secondTop	= heapbox.find(".heapOptions li a").eq(1).offset().top;
			eachTop		= parseInt(secondTop) - parseInt(firstTop);
			selectedTop = $(heapbox.find(".heapOptions li a.selected")).offset().top;
			lastTop		= heapbox.find(".heapOptions li a").last().offset().top;
			totalTop    = parseInt(selectedTop) - parseInt(firstTop) + parseInt(eachTop);
			lastScroll  = parseInt(lastTop) - parseInt(this.options.heapsize,10);
			
			if(direction=='down') {
				if(parseInt(totalTop) > parseInt(this.options.heapsize,10) && parseInt(heap.scrollTop()) < parseInt(totalTop) - parseInt(this.options.heapsize,10)) {
					heap.animate({ scrollTop: parseInt(totalTop) - parseInt(this.options.heapsize,10) }, 10);
				};
			}
			else if(direction=='up') {						
				sTop = parseInt(lastTop) - parseInt(firstTop) + ( 2 * parseInt(eachTop)) - parseInt(this.options.heapsize,10);
				if(parseInt(totalTop) < sTop && parseInt(heap.scrollTop()) > parseInt(totalTop) - parseInt(eachTop)) {
					heap.animate({ scrollTop: parseInt(totalTop) -  parseInt(eachTop)}, 10);				
				};
			}
		}
	...
	/*
		search Heap by모두복지몰.kr
	*/
	_searchHeap: function(vls) {	
		heapBoxEl = $("#heapbox_"+this.instance.heapId);
		
		var cnt = 0;
		heapBoxEl.find("div.heap ul li").each(function(){
			if($(this).find("a").attr('title').indexOf(vls)==-1) {
		...
		if(cnt > 0) {		
			firstTop	= heapBoxEl.find(".heapOptions li a").first().offset().top;
			if(cnt==1) {
				thisHeight	= '33px';
			}
			else {
				secondTop	= heapBoxEl.find(".heapOptions li a").eq(1).offset().top;
				eachTop		= parseInt(secondTop) - parseInt(firstTop);
				if(eachTop < 1) eachTop = 33;
				thisHeight	= eachTop * cnt;
			}
					
			heap = $("#heapbox_"+this.instance.heapId+" .heap");	
			
			if(parseInt(thisHeight) < parseInt(heap.height())) {			
				heap.css({"height":parseInt(thisHeight)+'px'});		
			}	
		}
	...
	/*
		search reset Heap by모두복지몰.kr
	*/
	_searchResetHeap: function() {
		heapBoxEl = $("#heapbox_"+this.instance.heapId);
		heapBoxEl.find("div.heap ul li").each(function(){
			$(this).css('display', 'block');
		});		
		
		$("#heapbox_"+this.instance.heapId+" .heap").css({"height":this.tmpHeapSize, "overflow":"auto"});
		$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('.heapResetIcon').hide();		
		$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('.heapSearchIcon').show();
	},
	...
	*/
	_closeheap: function(internal,closeStartEvent,closeCompleteEvent) {
		if(this.openYN=='N') return;
		
		if(this.searchClick) {			
			$("div#heapbox_"+this.instance.heapId+" .heapSearch").find('input:text').focus();					
			return;
		}
		...
		if($("div#heapbox_"+this.instance.heapId+" .heapSearch").length) {
			$("div#heapbox_"+this.instance.heapId+" .heapSearch").hide();
		}		
		if($("div#heapbox_"+this.instance.heapId+" .handler i").hasClass("rotate180")) $("div#heapbox_"+this.instance.heapId+" .handler i").addClass('rotate180r');			
		$("div#heapbox_"+this.instance.heapId+" .handler i").removeClass('rotate180');	
	...
	*/
	_openheap: function() {
		...
		if($("div#heapbox_"+this.instance.heapId+" .heapSearch").length) {
			$("div#heapbox_"+this.instance.heapId+" .heapSearch").show();
		}	
		
		//$("div#heapbox_"+this.instance.heapId+" .handler i").rotate({animateTo:180,duration: 200});
		$("div#heapbox_"+this.instance.heapId+" .handler i").removeClass('rotate180r');
		$("div#heapbox_"+this.instance.heapId+" .handler i").addClass('rotate180');
		if(this.searchUsed)	$("div#heapbox_"+this.instance.heapId+" .heapSearch").show();
	...
	*/
	...
	update: function() {
		if(!this.searchUsed) this._setDefaultValues();
		else {

			var self = this;
			$("#heapbox_"+this.instance.heapId).find(".heap ul li a").removeClass("selected").first().addClass("selected");
