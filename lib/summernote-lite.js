 ...
 *
 * Added ability to add links to images BY모두복지몰.kr
 * 
 */
...
(function (global, factory) {
  ...
  var TooltipUI = /** @class */ (function () {
      function TooltipUI($node, options) {
          ...
          this.$tooltip = $([
              ...
              '  <div class="note-tooltip-content fontNoto"/>',
              '</div>',
          ].join(''));
          // define event
  ...
  var airEditable = renderer.create([
      '<div class="note-editable ip_style" contentEditable="true" role="textbox" aria-multiline="true" />',
      '<output class="note-status-output" role="status" aria-live="polite" />',
  ].join(''));
  var airEditable2 = renderer.create([
      '<div class="note-editable ip_style2" contentEditable="true" role="textbox" aria-multiline="true" />',
      '<output class="note-status-output" role="status" aria-live="polite" />',
  ].join(''));
  ...
  var imageLinkDialog = function (opt) {
     var body = '<div class="form-group">' +
      ...
      var footer = '<button href="#" class="btn btn-primary note-link-btn disabled" disabled>' + opt.lang.image.insertLink + '</button>';
	  
      return dialog({
          className: 'note-image-link-dialog',
          title: opt.lang.image.insertLink,
          fade: opt.fade,
          body: body,
          footer: footer
      }).render();
  ...
  var popover2 = renderer.create([
      '<div class="note-popover2 bottom">',
      '  <div />',
      '  <div class="popover-content note-children-container"/>',
      '</div>',
  ].join(''), function ($node, options) {
  ...
  var ui = {
      ...
      createLayout: function ($note, options) {
		  if(options.airPos=="air_pos_detail") {
  ...
  $$1.extend($$1.summernote.lang, {
      'en-US': {
          ...
          help: {
			  ...
			  'imageLinkDialog.show': 'Show Image Link Dialog'
          },
          history: {
              undo: 'Undo',
              redo: 'Redo'
  ...
  var Context = /** @class */ (function () {
      ...
      Context.prototype.createInvokeHandler = function (namespace, value) {
		  ...
		  if(namespace =='imageLinkDialog.show')
		  {	
			  return _this.invoke(namespace, value || _this.targets.closest('[data-value]').data('value'), _this.targets);
		  }
      ...
      Context.prototype.invoke = function () {		  
		  ...
		  if(methodName=='justifyLeft' || methodName=='justifyCenter' || methodName=='justifyRight' || methodName=='justifyFull' || methodName=='outdent' || methodName=='indent') {
			  TBui.toggleBtnActive(TB.find('.btn-paragraph'), 0);	 	
		  }
		  
		  if(methodName=='fontName') {
			  TBui.toggleBtnActive(TB.find('.btn-fontName'), 0);	 	
		  }

		  if(methodName=='fontSize') {
			  TBui.toggleBtnActive(TB.find('.btn-fontSize'), 0);	 	
		  }

		  if(methodName=='formatBlock') {
			  TBui.toggleBtnActive(TB.find('.btn-style'), 0);	 	
		  }
	   ...
	   Context.prototype.airObject = function() {		   
		   AIR_OBJECT[this.options.airPos].css({
              left: $('#'+this.options.airPos).position().left + $('#contentBox').position().left + 16,
              top: $('#'+this.options.airPos).position().top  + $('#contentBox').position().top + 16
           });
  ...
  */
  ...
  var Editor = /** @class */ (function () {
      function Editor(context) {
		  ...
		  /**
			...
			this.unlinkImageLink = function ($editable, value, $target) {
				...
				if($target.parent('a').length){
					$target.unwrap();
					$(".createImageLink").show();
				    $(".unlinkImageLink").hide(); 
				}
		...
		/**
		  ...
		  this.createImageLinkInit = this.wrapCommand(function () { 
			 var $target = $$1(_this.restoreTarget());

			 _this.context.targets = $target;

			  _this.context.createInvokeHandler('imageLinkDialog.show');
			  
		  });


          this.createImageLink = this.wrapCommand(function (linkInfo) { 
				var tmps	= linkInfo.url.split('|');
			 	var linkUrl = tmps[0];
				var isNewWindow = tmps[1];
				var targets = _this.context.targets;
				...
				if(targets.parent('a').length) {
					var anchor = targets.parent('a');										
				} 
				else {
					var anchor = $('<a></a>');
				}
          ...
          this.insertTable = this.wrapCommand(function (dim) {
			  ...
			  TBui.toggleBtnActive(TB.find('.btn-table'), 0);
          });
          /**
          ...
          this.removeMedia = this.wrapCommand(function () {
			  ...
			  if(_this.options.imgSync) imageStatus();
          });
          /**
           * float me
           *
      ...
      Editor.prototype.initialize = function () {
          ...
          this.$editable.on('keydown', function (event) {
              if (event.keyCode === key.code.ENTER) {				  
				  if(_this.options.airMode) {
					  if(typeof(_this.options.nextFocus)=='string')  $(_this.options.nextFocus).summernote({focus: true});
					  else _this.options.nextFocus.focus();
					  return false;
				  }
			  ...
			  if(event.keyCode!='37' && event.keyCode!='39' && event.keyCode!='8' && event.keyCode!='46') {				  
				if(_this.tmpStr) {				
					_this.$editable.text(_this.tmpStr);			
					_this.$editable.blur();
				}
				else {
					 if (_this.options.maxTextLength > 0) {			  			  						 
						if ((_this.$editable.text().length) > _this.options.maxTextLength) {				 							
							  _this.$editable.text(_this.$editable.text().substring(0,_this.options.maxTextLength));
							  _this.tmpStr = _this.$editable.text();							  
							  _this.$editable.blur();
			  ...
			  if(_this.options.airPos) {
				$('#'+_this.options.airPos+'_cnt').html(_this.$editable.text().length);
			  }
      ...
      Editor.prototype.isLimited = function (pad, event) {
          ...
          if (this.options.maxTextLength > 0) {			  			  
			  if ((this.$editable.text().length + pad) == this.options.maxTextLength) {				 
				  this.tmpStr = this.$editable.text();								
			  }
			  else if ((this.$editable.text().length + pad) > this.options.maxTextLength) {				 
				  this.tmpStr = this.$editable.text().substring(0,this.options.maxTextLength);
			  }
			  else if ((this.$editable.text().length + pad) < this.options.maxTextLength) {				 
				  this.tmpStr = '';
			  }
      ...
      */
      ...
      Editor.prototype.insertImage = function (src, param) {
          ...
          return createImage(src, param).then(function ($image) {
			  ...
			  $(a).append($image[0]);
              var node = $$1('<a />').html($(a).html()).attr('href', _this.options.linkUrl)[0];
			  $$1(node).attr('target', '_blank');              
			  range.create(_this.editable).insertNode(node);
			  */

			  range.create(_this.editable).insertNode($image[0]);
			  range.create(_this.editable).insertNode(dom.create('BR'));
  ...
  var CodeView = /** @class */ (function () {
      ...
      CodeView.prototype.toggle = function () {
          if (this.isActivated()) {
              if(this.options.imgSync) imageStatus();
              this.deactivate();
          }
          else {
              this.activate();
      ...
      Statusbar.prototype.initialize = function () {
          ...
          this.$statusbar.on('mousedown', function (event) {
              ...
              var onMouseMove = function (event) {
				  ...
				  if($('#air_pos_detail')) {
					   AIR_OBJECT['air_pos_detail'].css({
						  left: $('#air_pos_detail').position().left + $('#contentBox').position().left + 16,
						  top: $('#air_pos_detail').position().top  + $('#contentBox').position().top + 16
					   });
				  }
  ...
  var Buttons = /** @class */ (function () {
      ...
      Buttons.prototype.addToolbarButtons = function () {
          ...
          this.context.memo('button.style', function () {
              return _this.ui.buttonGroup([
                  _this.button({
                      className: 'dropdown-toggle btn-style',
          ...
          this.context.memo('button.fontname', function () {
              ...
              return _this.ui.buttonGroup([
                  _this.button({
                      className: 'dropdown-toggle btn-fontName',
          ...
          this.context.memo('button.fontsize', function () {
              return _this.ui.buttonGroup([
                  _this.button({
                      className: 'dropdown-toggle btn-fontSize',
          ...
          this.context.memo('button.paragraph', function () {
              return _this.ui.buttonGroup([
                  _this.button({
                      className: 'dropdown-toggle btn-paragraph',
          ...
          this.context.memo('button.table', function () {
              return _this.ui.buttonGroup([
                  _this.button({
                      className: 'dropdown-toggle btn-table',
      ...
      Buttons.prototype.addImagePopoverButtons = function () {
		  ...
		  this.context.memo('button.showImageLink', function () {
              return _this.button({
                  contents: _this.ui.icon(_this.options.icons.link),
                  tooltip: _this.lang.image.insertLink,
			      className: 'createImageLink',
                  click: _this.context.createInvokeHandler('editor.createImageLinkInit')  //imageLinkDialog.show
              }).render();
          });
		  this.context.memo('button.unlinkImageLink', function () {
              return _this.button({
                  contents: _this.ui.icon(_this.options.icons.unlink),
                  tooltip: _this.lang.image.unlink,
				  className: 'unlinkImageLink',
                  click: _this.context.createInvokeHandler('editor.unlinkImageLink')
              }).render();
          });
      };
          ...
          var body = [
              ...
              !this.options.disableLinkTarget
                  ? $$1('<div/>').append(this.ui.checkbox({
                      ...
                      text: '<span></span> ' + this.lang.link.openInNewWindow,
                      checked: true
                  }).render()).html()
                  : '',
          ].join('');
  ...
  var ImageLinkDialog = /** @class */ (function () {
      function ImageLinkDialog(context) {
          this.context = context;
		  this.layoutInfo = context.layoutInfo;
          this.ui = $$1.summernote.ui;
          this.$body = $$1(document.body);
          this.$editor = context.layoutInfo.editor;
          this.options = context.options;
       ...
       * toggle update button
       */
      ImageLinkDialog.prototype.toggleLinkBtn = function ($linkBtn, $linkUrl) {
          this.ui.toggleBtn($linkBtn, $linkUrl.val());
      };
      /**
       * Show link dialog and set event handlers on dialog controls.
       *
       ...
       * @return {Promise}
       */
      ImageLinkDialog.prototype.showImageLinkDialog = function (linkInfo) {
          var _this = this;

		  return $$1.Deferred(function (deferred) {
			  
			  //var $linkDialog = _this.$dialog.find('.note-image-link-dialog');
			  var $linkDialog = _this.$dialog;
              ...
              _this.ui.onDialogShown(_this.$dialog, function () {
                  ...
                  $linkUrl.on('input paste propertychange', function () {
                      ...
                      //_this.toggleLinkBtn($linkBtn, $linkUrl);					 
                  }).val(linkInfo.url);
                  if (!env.isSupportTouch) {
                      $linkUrl.trigger('focus');
                  }
                 // _this.toggleLinkBtn($linkBtn, $linkUrl);
                  ...
                  $linkBtn.one('click', function (event) {
                      ...
                      deferred.resolve({
                          range: linkInfo.range,
                          url: $linkUrl.val() + '|' + $openInNewWindow.is(':checked'),                         
                      });
					  $linkUrl.val(''); 
                      _this.ui.hideDialog(_this.$dialog);
                  });
      ...
      ImageLinkDialog.prototype.show = function () {
		 ...
		 var $editor = _this.layoutInfo.editor,
			  $handle = Handle.$handle,
			  $editable = _this.layoutInfo.editable;

          var linkInfo = this.context.invoke('editor.getLinkInfo');
          this.context.invoke('editor.saveRange');
          this.showImageLinkDialog(linkInfo).then(function (linkInfo) {			  
              _this.context.invoke('editor.restoreRange');
              _this.context.invoke('editor.createImageLink', linkInfo);
          }).fail(function () {
              _this.context.invoke('editor.restoreRange');
          });		  
  ...
  var LinkPopover = /** @class */ (function () {
      ...
      LinkPopover.prototype.update = function () {
          ...
          if (rng.isCollapsed() && rng.isOnAnchor()) {
			  ...
			  var imgs = $$1(anchor).attr('data-img');
			  var dp = 'block';

			 if(imgs == '1') var dp = 'none';			  
  ...
  var ImageDialog = /** @class */ (function () {
      ...
      ImageDialog.prototype.initialize = function () {
		  ...
		  var body2 = [              
              '<div class="form-group note-group-image-url" style="overflow:auto;">',
              '<label class="note-form-label">링크주소</label>',
              '<input class="note-image-url-link form-control note-form-control note-input ',
              ' col-md-12" type="text" />',
              '</div>',
          ].join('');
      ...
      ImageDialog.prototype.showImageDialog = function () {
          ...
          return $$1.Deferred(function (deferred) {
			  ...
			  var $imageUrlLink = _this.$dialog.find('.note-image-url-link');
              ...
              _this.ui.onDialogShown(_this.$dialog, function () {
				  ...
				  $imageUrlLink.on('input paste propertychange', function () {
                      _this.options.linkUrl = $(this).val();					  
                  }).val('');
                  if (!env.isSupportTouch) {
  ...
  var ImagePopover = /** @class */ (function () {
      ...
      ImagePopover.prototype.update = function (target, event) {
          if (dom.isImg(target)) {
			  ...
			  if($(target).parent('a').length) {
				$(".createImageLink").hide();
				$(".unlinkImageLink").show(); 
			  }
			  else {
				$(".createImageLink").show();
				$(".unlinkImageLink").hide();   
  ...
  var AirPopover = /** @class */ (function () {
      ...
      AirPopover.prototype.initialize = function () {
          this.$popover = this.ui.popover2({
              className: 'note-air-popover'
          }).render().appendTo(this.options.container);
		  AIR_OBJECT[this.options.airPos] = this.$popover;
      ...
      options: {
          ...
          popover: {
              image: [
                  ...
                  ['float', ['floatLeft', 'floatRight', 'floatNone']],
                  ['remove', ['removeMedia']],
				  ['showImageLink', ['showImageLink', 'unlinkImageLink']],
              ],
              link: [
		  ...
		  */
		  fontNames: [
              'Arial', 'Arial Black', 'Comic Sans MS', 'Impact', 'Tahoma', '굴림', '돋움', 'Noto Sans KR', 'SCDream',
          ],
          fontNamesIgnoreCheck: ['SCDream'],
