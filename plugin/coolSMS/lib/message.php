...
$config_sms = get_config($coolsms_key, $coolsms_secret);

function get_header() {
  global $config_sms;
  $apiKey		= $config_sms["apiKey"];
  $apiSecret	= $config_sms["apiSecret"];
...
function request($method, $resource, $data = false) {
  global $config_sms;
  $url = "{$config_sms['protocol']}://{$config_sms['domain']}";
  if ($config_sms['prefix']) $url .= $config_sms['prefix'];
  $url .= $resource;
...
function send_messages_many($messages) {
  $params = array(
    "agent" => array(
