...
$sql	= "SELECT * FROM mallRN_configuration_social WHERE site = 'PAYCO'";
$data	= $mysql->one_row($sql);

if($data['used'] != 1) {
	if(checkGetVar('logout') == 1) Header ('Location: ../../');
	else alert("PAYCO 아이디로 로그인 미사용 상태 입니다.", "close");
}

if(!$data['api_id'] || !$data['api_key']) {
	alert("PAYCO 아이디로 로그인 정보가 등록되지 않았습니다.", "close");
}
...
try {
    ...
    $adapter		= $hybridauth->authenticate('Payco');
	...
	$birth = $userProfile->birthDay;
	if($userProfile->gender == 'FEMALE') $userProfile->gender = 'F';
	else if($userProfile->gender == 'MALE') $userProfile->gender = 'M';
	else $userProfile->gender = 'N';
	...
	echo "<form name='registForm' action='../../../php/popup_sns_regist.php' method='post'>\n
			...
			<input type='hidden' name='sns_type' value='payco' />\n
		 </form>\n
