...
$sql	= "SELECT * FROM mallRN_configuration_social WHERE site = 'NAVER'";
$data	= $mysql->one_row($sql);

if($data['used'] != 1) {
	if(checkGetVar('logout') == 1) Header ('Location: ../../');
	else alert("NAVER 아이디로 로그인 미사용 상태 입니다.", "close");
}

if(!$data['api_id'] || !$data['api_key']) {
	alert("NAVER 아이디로 로그인 정보가 등록되지 않았습니다.", "close");
}
...
try {
    ...
    $adapter		= $hybridauth->authenticate('Naver');
	...
	$birth = $userProfile->birthYear.str_replace("-", "", $userProfile->birthDay);
