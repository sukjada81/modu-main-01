...
//*******************************************************************************
  ...
  if($PGIP == "203.238.37.15" || $PGIP == "118.129.210.25" || $PGIP == "183.109.71.153")	//PG에서 보냈는지 IP로 체크
  {
		...
		if($data) {		
			$card_info = $data['pay_info'].", 입금자명 : {$P_UNAME}, 입금금액 : {$P_AMT}";		
			
			$sql = "UPDATE mallRN_order_info SET pay_status = 'C' , pay_info = '{$card_info}', pay_number = '{$P_TID}' WHERE order_num = '{$order_num}'";
...
function writeLog($msg)
{
    ...
    if(!($fp = fopen($_SERVER['DOCUMENT_ROOT']."/".CONF_ROOT."plugin/inicis/log/".CONF_KEY.$file, "a+"))) return 0;
