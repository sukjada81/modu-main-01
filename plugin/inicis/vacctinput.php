...
$INIpayHome = $_SERVER['DOCUMENT_ROOT']."/".CONF_ROOT."plugin/inicis/log/".CONF_KEY;      // 이니페이 홈디렉터리
//**********************************************************************************
...
if ($PG_IP == "203.238.37" || $PG_IP == "39.115.212" || $PG_IP == "183.109.71") {  //PG에서 보냈는지 IP로 체크
    ...
    $logfile = fopen($INIpayHome . "/result.log", "a+");
...
$sql	= "SELECT * FROM mallRN_order_info WHERE pay_type = 'V' && order_num = '{$order_num}'";
$data	= $mysql->one_row($sql);

if($data) {		
	$card_info = $data['pay_info'].", 입금자명 : {$nm_input}, 입금금액 : {$amt_input}";		
	if($no_cshr_tid) $card_info .= ", 현금영수증 승인번호 : {$no_cshr_appl}";

	$sql = "UPDATE mallRN_order_info SET pay_status = 'C' , pay_info = '{$card_info}', pay_number = '{$no_tid}' WHERE order_num = '{$order_num}'";
