...
function goodsDel($uid) {
	global $mysql, $v_my_id;

	$img_block = floor($uid/10000);

	$sql = "SELECT * FROM mallRN_goods WHERE vendor = '{$v_my_id}' && uid = '{$uid}'";
...
function goodsCopy($orig_uid) {
	global $mysql, $item_array, $v_my_id, $goods_auth;
	...
	$sql = "SELECT * FROM mallRN_goods WHERE vendor = '{$v_my_id}' && uid = '{$orig_uid}'";
	if(!$data = $mysql->one_row($sql)) logMsg("해당상품은 삭제 되었거나 존재하지 않습니다.");
	
	if($goods_auth == 'P')	$data['auth_ck'] = "N";
	else					$data['auth_ck'] = "Y";
	...
	$sql .= " commission = '{$data['commission']}', commission_type = '{$data['commission_type']}'";
	$mysql->query($sql);
	$uid	= $mysql->InsertNo();	
	...
	$sql = "UPDATE mallRN_goods SET image1 = '{$image1}', image2 = '{$image2}', image3 = '{$image3}', re_uid = '{$re_uid}', vendor = '{$v_my_id}', signdate='{$signdate}' WHERE uid='{$uid}'";
	$mysql->query($sql);
	######################## 이미지 이름변경  #########################	
...
if($mode=='write' || $mode=='modify') {
	...
	for($i=1;$i<4;$i++) {
		...
		if($i>1) {
			if($mode=='write' && !$_POST['image'.$i] && $base_image) {
	...
	$_POST['related_goods_type'] = 1;
}

$sql		= "SELECT goods_auth, sell, commission FROM mallRN_vendor WHERE id = '{$v_my_id}'";
$vdata		= $mysql->one_row($sql);
$goods_auth	= $vdata['goods_auth'];
$sell		= $vdata['sell'];

$_POST['vendor']		= $v_my_id;
$_POST['commission']	= $vdata['commission'];

if($goods_auth == 'P')	$_POST['auth_ck'] = "N";
else					$_POST['auth_ck'] = "Y";

if($sell == 'A')		$_POST['vendor_hide'] = "0";
else					$_POST['vendor_hide'] = "1";

unset($vdata);

$item_array			= array('cate', 'name', 'name_code_able', 'price', 'orig_price', 'consumer_price', 'price_ment', 'other_image', 'detail_image', 'detail_image_only', 'detail_image_type', 'explains', 'option_use', 'option_info', 'qty_type', 'qty', 'limit_qty', 'require_info', 'display_use', 'sale_use', 'store_display1', 'store_display2', 'store_display3', 'icon', 'goods_code', 'detail', 'brand', 'make', 'origin', 'model', 'making_info', 'delivery_type', 'delivery_type_qty', 'delivery_price', 'keyword', 'related_goods_type', 'information_use', 'delivery_info', 'refund_info', 'exchange_info', 'as_info', 'auth_ck','delivery_im_areas1_used','delivery_im_areas1_price','delivery_im_areas2_used','delivery_im_areas2_price');
$item_default		= array('detail_image_only', 'option_use', 'qty_type', 'display_use', 'sale_use', 'store_display1', 'store_display2', 'store_display3','delivery_im_areas1_used','delivery_im_areas1_price','delivery_im_areas2_used','delivery_im_areas2_price');
$item_able_value	= array('detail_image_type' => ['1', '2'], 'related_goods_type' => ['0', '1', '2', '3', '4'], 'delivery_type' => ['1', '2', '3', '4', '5'], 'information_use' => ['1', '2', '3']);
...
switch($mode) {
	...
	case "display" :		
		...
		$sql = "UPDATE mallRN_goods SET display_use = '{$value}', moddate='{$moddate}' WHERE vendor = '{$v_my_id}' && uid IN (".join(",",$item).")";		
		$mysql->query($sql);
	...
	case "sale" :
		...
		$cnt		= count($item);
		if(!$item || strlen($value) == 0)  logMsg($value.'필수 정보가 제대로 넘어오지 못했습니다.');
				
		$sql = "UPDATE mallRN_goods SET sale_use = '{$value}', moddate='{$moddate}' WHERE vendor = '{$v_my_id}' && uid IN (".join(",", $item).")";
		$mysql->query($sql);
		$cnt = number_format($mysql->affected_rows());
	...
	case "modOne" :
		...
		$item_array = array('price', 'orig_price', 'consumer_price', 'qty_type', 'qty');
		...
		$sql .= " auth_ck = '{$_POST['auth_ck']}', moddate = '{$moddate}' WHERE vendor = '{$v_my_id}' && uid = '{$uid}'";
		$mysql->query($sql);
	...
	case "delivery" :
		...
		if(!preg_match("/b./i",$where)) {
			...
			$sql = "UPDATE mallRN_goods a, mallRN_goods_cate b SET
					...
					a.delivery_im_areas2_price	= '{$delivery_im_areas2_price}',
					a.moddate					= '{$moddate}'
				WHERE a.vendor = '{$v_my_id}' && a.uid = b.guid {$where}
				";
		}		
		$mysql->query($sql);
	...
	case "price" :
		...
		$sql				= "SELECT commission FROM mallRN_vendor WHERE id = '{$v_my_id}'";
		$vendor_commission	= $mysql->get_one($sql);
		...
		if($price) {
			...
			if(!preg_match("/b./i",$where)) {
				$sql = "SELECT a.uid, a.orig_price, a.commission, a.commission_type, a.vendor FROM mallRN_goods a WHERE a.vendor ='{$v_my_id}' {$where}";
			}
			else {
				$sql = "SELECT a.uid, a.orig_price, a.commission, a.commission_type, a.vendor FROM mallRN_goods a, mallRN_goods_cate b WHERE a.vendor ='{$v_my_id}' && a.uid = b.guid {$where} GROUP BY a.uid";
			}	
			...
			if($mysql->affected_rows()>0) {
				...
				while($row = $mysql->fetch_array()) {
					...
					else $commission = $vendor_commission;
					$def_price = priceLimit($price2 * 100 / (100 - $commission));

					$sql = "UPDATE mallRN_goods SET price = '{$def_price}', orig_price = '{$price2}', auth_ck = '{$_POST['auth_ck']}', moddate = '{$moddate}' WHERE uid = '{$row['uid']}'";
					$mysql->query2($sql);
				}
			}
		...
		if($price) {			
			...
			if(!preg_match("/b./i",$where)) {
				$sql = "SELECT a.uid, a.price, a.commission, a.commission_type, a.vendor FROM mallRN_goods a WHERE a.vendor ='{$v_my_id}' {$where}";
			}
			else {
				$sql = "SELECT a.uid, a.price, a.commission, a.commission_type, a.vendor FROM mallRN_goods a, mallRN_goods_cate b WHERE a.vendor ='{$v_my_id}' && a.uid = b.guid  {$where} GROUP BY a.uid";
			}	
			...
			if($mysql->affected_rows()>0) {
				...
				while($row = $mysql->fetch_array()) {
					...
					$sql = "UPDATE mallRN_goods SET price = '{$price2}', orig_price = '{$orig_price}', auth_ck = '{$_POST['auth_ck']}', moddate = '{$moddate}' WHERE uid = '{$row['uid']}'";
					$mysql->query2($sql);
				}
			}
	...
	case "image" :
		...
		if(!preg_match("/b./i",$where)) {
			$sql = "SELECT a.uid, a.image1, a.image2, a.image3, a.other_image FROM mallRN_goods a WHERE a.vendor = '{$v_my_id}' && {$where}";
		}
		else {
			$sql = "SELECT a.uid, a.image1, a.image2, a.image3, a.other_image FROM mallRN_goods a, mallRN_goods_cate b WHERE a.vendor = '{$v_my_id}' && a.uid = b.guid {$where} GROUP BY a.uid";
		}		
		$mysql->query($sql);
		$cnt =  $mysql->affected_rows();
	...
	case "option" :
		...
		######################## 상품 옵션 정보  #########################	
		$sql = "UPDATE mallRN_goods SET moddate='{$moddate}', option_soldout = '{$option_soldout}' WHERE vendor = '{$v_my_id}' && uid='{$uid}'";		
		$mysql->query($sql);
	...
	case "cate" :
		...
		if(!preg_match("/b./i", $where)) {
			$sql = "SELECT a.uid FROM mallRN_goods a WHERE a.vendor = '{$v_my_id}' {$where} ORDER BY a.uid ASC";
		}
		else {
			$sql = "SELECT DISTINCT(a.uid) FROM mallRN_goods a, mallRN_goods_cate b WHERE a.vendor = '{$v_my_id}' && a.uid = b.guid {$where} ORDER BY a.uid ASC";
		}
		$mysql->query($sql);
	...
	case "excel" :
		...
		if($matching==1) {
			$sql = "SELECT cate, used FROM mallRN_cate WHERE cate_sub = '0' ORDER BY cate ASC";
			$mysql->query($sql);
		...
		$item_array2 = array('cate','name','name_code_able','price','orig_price','consumer_price','explains','qty_type','qty','require_info','display_use','sale_use','goods_code','detail','brand','make','origin','model','delivery_type','delivery_price','keyword','cate_hide','vendor_hide','auth_ck','commission','vendor','signdate');

		$auth_ck		= $_POST['auth_ck'];
		$vendor_hide	= $_POST['vendor_hide'];
		...
		for($l = 2, $cnt = 0; $l <= $num_rows; $l++) {			
			...
			if(!$price && !$orig_price) continue;
			if(!$name || !$cate) continue;
			$cnt++;

			if($price && $orig_price) $orig_price = "";

			if($price) {
				$orig_price = priceLimit($price * ((100 - $commission) / 100));
			}
			else {
				$price		= priceLimit($orig_price * 100 / (100 - $commission));
			}
    ...
    case "write" : 
		...
		array_push($item_array, 'image1', 'image2', 'image3', 'cate_hide', 'vendor_hide', 'vendor', 'commission', 'signdate');
