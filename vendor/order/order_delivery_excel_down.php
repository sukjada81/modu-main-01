...
if($status1 == 1 && $status2 == 1) $where = " && (c.status = 1 || c.status = 2)";
else if($status1 == 1) $where = " && c.status = 1";
else if($status2 == 1) $where = " && c.status = 2";

$sql	= "SELECT c.*, b.name2, b.cell2, b.postcode, b.address1, b.address2, b.message FROM ( SELECT a.order_num, a.name2, a.cell2, a.postcode, a.address1, a.address2, a.message FROM mallRN_order_info a WHERE a.reals = 1 ) b JOIN mallRN_order_goods c ON b.order_num = c.order_num && c.vendor_delivery = '{$v_my_id}' {$where} ORDER BY c.uid ASC";
$mysql->query($sql);
