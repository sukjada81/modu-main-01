...
for($i = 0; $i < 3; $i ++) {
	if($i < 2) {
		$sql					= "SELECT SUM(IF(mobile = 'N' && status = 0, price , 0)) as p_total1, SUM(IF(mobile = 'N' && status = 1, price , 0)) as p_total2, SUM(IF(mobile = 'Y' && status = 0, price , 0)) as m_total1, SUM(IF(mobile = 'Y' && status = 1, price , 0)) as m_total2 FROM mallRN_order_sales WHERE {$where} && type = '{$i}' {$where2}";
	}
	else {
		$sql					= "SELECT SUM(IF(mobile = 'N' && status = 0, commission , 0)) as p_total1, SUM(IF(mobile = 'N' && status = 1, commission , 0)) as p_total2, SUM(IF(mobile = 'Y' && status = 0, commission , 0)) as m_total1, SUM(IF(mobile = 'Y' && status = 1, commission , 0)) as m_total2 FROM mallRN_order_sales WHERE {$where} && type = '0' {$where2}";
	}
