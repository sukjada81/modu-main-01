...
//if(CONF_ROOT) $_SERVER['REQUEST_URI'] = str_replace(CONF_ROOT, "", $_SERVER['REQUEST_URI']);
...
$sql			= "SELECT widget_info, push_yn FROM mallRN_vendor_configuration WHERE vendor = '{$v_my_id}'";
$vendor_info	= $mysql->one_row($sql);	

$push_checked	= "";
if($vendor_info['push_yn'] == 'Y') $push_checked = "checked='checked'";
$widget_info	= $vendor_info['widget_info'];


$ck_sub	= 0;
if(preg_match("/info.php/i",$URI_NAME_ARR[3])) {
	$mode					= isset($_GET['mode']) ? $_GET['mode'] : '';
	$info_list_change_arr	= array('board', 'inquiry', 'order');
	$ck						= 0; 
...
if("{$URI_NAME_ARR[2]}/{$URI_NAME_ARR[3]}" == "main/index.php") {
	...
	if(!$widget_info) {
		$widget_info = "order,goods,count,sales,order_step,order_list,notice,board,board_vnotice,calculate,tips";
		$sql			= "UPDATE mallRN_vendor_configuration SET widget_info = '{$widget_info}' WHERE vendor = '{$v_my_id}'";
		$mysql->query($sql);
	}
		...
		if($j==$MENU_SUB_NUM) {
			...
			if(@$MENU_ARR[$MENU_NUM][$j][3]) {				
				...
				if($SUB_MENU_SEC == 2) {
					...
					$sql	= "SELECT SUM(IF(status = '0' , 1 , 0)) as cnt0, SUM(IF(status = '1' , 1 , 0)) as cnt1, SUM(IF(status = '2' , 1 , 0)) as cnt2, SUM(IF(status = '3' , 1 , 0)) as cnt3, SUM(IF(status = '4' , 1 , 0)) as cnt4, SUM(IF(status = '5' , 1 , 0)) as cnt5, SUM(IF(status = '7' , 1 , 0)) as cnt6, SUM(IF(status = '8' , 1 , 0)) as cnt7, SUM(IF(status = '9' , 1 , 0)) as cnt8 FROM mallRN_order_goods WHERE reals = 1 && vendor = '{$v_my_id}'";
				...
				if($PARENT_SEC_NUM) {
					...
					if($ck_sub == 0) $tpl->parse("is_content_sub");
				}
			}
		}
...
$item_array = array('push_apiKey', 'push_authDomain', 'push_projectId', 'push_storageBucket', 'push_messagingSenderId', 'push_appId', 'push_server_key', 'push_server_key2', 'mobile_icon', 'basic_name');
$item_field	= join(", ", $item_array );

$sql		= "SELECT {$item_field} FROM mallRN_configuration WHERE uid = 1";
$shop_data	= $mysql->one_row($sql);

$ck			= 0;
foreach($item_array as $k => $v) {
	if($v != 'mobile_icon' && $v != 'basic_name' && !$shop_data[$v]) {
		$ck = 1;
		break;
	}
}

if($SITE_NAME = $shop_data['basic_name']) { 
	$tpl->parse("is_logo_name");
}
else $tpl->parse("is_logo_default");

if($shop_data['mobile_icon'])  {
	$mobile_icon = $HeaderProtocol.$_SERVER['HTTP_HOST']."/".CONF_ROOT."image/mobile/".stripslashes($shop_data['mobile_icon']);
}
else $mobile_icon = "";

if($ck == 0) {
	$tpl->parse("is_web_push");
}

unset($item_array, $item_field, $ck);
