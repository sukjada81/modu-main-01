...
$tpl->define("main","calculate_statistics.html");
$tpl->scan_area("main");
...
$sql = "SELECT signdate FROM mallRN_order_sales WHERE vendor = '{$v_my_id}' ORDER BY uid ASC LIMIT 1";
...
$addstring	= "";
if($vendor)	$addstring	= "vendor={$vendor}";
######################## 변수 정의 #############################
...
switch($mode) {
	case "year" :
		$where		= "1 = 1";
		$rwhere		= "INSTR(from_unixtime(confirm_date), '{{RCODE}}')";	
		$start		= $start_year;
		$end		= $this_year;
		$addstr1	= "";
		$addstr2	= "";
	...
	case "month" :
		...
		$rwhere		= "INSTR(from_unixtime(confirm_date), '{$year}-{{RCODE}}')";	
		$start		= 1;
		$end		= 12;
		$addstr1	= "{$year}-";
		$addstr2	= "";
	...
	case "week" :
		...
		else if($month == 'all')	$where		= "INSTR(from_unixtime(confirm_date), '{$year}')";
		else if($week == 'all')		$where		= "INSTR(from_unixtime(confirm_date), '{$year}-{$month}')";
	...
	case "day" :
		...
		$rwhere		= "INSTR(from_unixtime(confirm_date), '{$year}-{$month}-{{RCODE}}')";
		$start		= 1;
		$end		= $last_day;
		$addstr1	= "{$year}-{$month}-";
		$addstr2	= "";
	...
	case "time" :
		...
		$rwhere		= "INSTR(from_unixtime(confirm_date), '{$year}-{$month}-{$day} {{RCODE}}')";
		$start		= 0;
		$end		= 23;
		$addstr1	= "";
		$addstr2	= ":00";
...
for($i = 0; $i < 3; $i ++) {
	if($i < 2) {
		$sql					= "SELECT SUM(IF(mobile = 'N' && status = 0, price , 0)) as p_total1, SUM(IF(mobile = 'N' && status = 1, price , 0)) as p_total2, SUM(IF(mobile = 'Y' && status = 0, price , 0)) as m_total1, SUM(IF(mobile = 'Y' && status = 1, price , 0)) as m_total2 FROM mallRN_order_sales WHERE {$where} && type = '{$i}' && confirmation = 1  {$where2}";
	}
	else {
		$sql					= "SELECT SUM(IF(mobile = 'N' && status = 0, commission , 0)) as p_total1, SUM(IF(mobile = 'N' && status = 1, commission , 0)) as p_total2, SUM(IF(mobile = 'Y' && status = 0, commission , 0)) as m_total1, SUM(IF(mobile = 'Y' && status = 1, commission , 0)) as m_total2 FROM mallRN_order_sales WHERE {$where} && type = '0' && confirmation = 1  {$where2}";
	}
	...
	$PTOTAL					= $p_total1 - $p_total2;
	$MTOTAL					= $m_total1 - $m_total2;
	$TOTAL					= $PTOTAL + $MTOTAL;
...
for($i = $start; $i <= $end; $i++) {
	...
	$sql = "SELECT SUM(IF(type = 0 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total0, SUM(IF(type = 1 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total1, SUM(IF(type = 0 && mobile = 'N', IF(status = 0, commission, -commission), 0)) as p_total2,  SUM(IF(type = 0 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total0, SUM(IF(type = 1 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total1, SUM(IF(type = 0 && mobile = 'Y', IF(status = 0, commission, -commission), 0)) as m_total2 FROM mallRN_order_sales WHERE {$rwhere2} && confirmation = 1 {$where2}";	
	$data = $mysql->one_row($sql);
	...
	$COUNTSTR[3][]				= $SALES_COUNTSTR;
	$COUNTSTRPC[3][]			= $SALES_COUNTSTRPC;
	$COUNTSTRMOBILE[3][]		= $SALES_COUNTSTRMOBILE;
