...
$table_name	= 'mallRN_store_count_referer';
...
$link_page	= "count_referer.php?{$addstring}";
...
switch($mode) {
	...
	case "delete" :
		...
		for($i = 0, $cnt = count($item); $i < $cnt; $i ++) {
			...
			$sql = "DELETE FROM {$table_name} WHERE vendor = '{$v_my_id}' && uid = '{$uid}'";
			$mysql->query($sql);
		}
		iframeViewMsg("{$i}건의 접속경로가 삭제 되었습니다!");
