...
######################## listPaging 정의 #############################
$listPaging						= new listPaging('mallRN_favorite_store'); 
$listPaging->search_variable	= array('limit' => 0, 'page' => 0, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'vendor' => 'function|uid', 'signdate' => 'date');
...
$vendor_function = function ($vls, $uid) {
	global $mysql, $tpl, $goods_field; 
	
	$sql					= "SELECT comp_name FROM mallRN_vendor WHERE id = '{$vls}'";
	$vinfo					= $mysql->one_row($sql);
	
	$sql					= "SELECT basic_name FROM mallRN_vendor_configuration WHERE vendor = '{$vls}'";
	$vshop_config			= $mysql->one_row($sql);

	$GLOBALS['STORE_NAME']	= !empty($vshop_config['basic_name'])	 ? stripslashes($vshop_config['basic_name']) : stripslashes($vinfo['comp_name']);

	$sql = "SELECT {$goods_field} FROM mallRN_goods WHERE display_use = 1 && auth_ck = 'Y' && cate_hide = 0 && vendor_hide = 0 && vendor = '{$vls}' ORDER BY store_display1 DESC, store_display2 DESC, store_display3 DESC, order_cnt DESC LIMIT 6";
	$mysql->query2($sql);
	...
	$sql					= "SELECT count(*) FROM mallRN_favorite_store WHERE vendor = '{$vls}'";
