...
if($reset == 0) {	
	...
	$total_stars2	= 0;
	for($i = 1; $i < 6; $i ++) {
		$sql			= "SELECT count(*) FROM mallRN_review WHERE vendor = '{$vendor}'";
	...
	$SEC_CATE_LOCATION	= "";
	if($cate_info['cate_dep'] > 1 || $mobile_header != '') $SEC_CATE_LOCATION	= str_replace("list", "store_cate&vendor={$vendor}", getCateAllName($cate, 2));

	################ SUB CATEGORY ################	
	if($cate_info['cate_sub'] == 0 && $cate_info['cate_dep'] > 1) $cate_info_dep = $cate_info['cate_dep'] - 1;
	else $cate_info_dep = $cate_info['cate_dep'];

	$ck_cate_dep	= $cate_info_dep * 3;
	$ck_cate_dep2	= $ck_cate_dep + 3;
	$ck_cate_dep3	= "";
	for($i = 0; $i < (12 - $ck_cate_dep); $i ++) $ck_cate_dep3 .= "0";

	$sql = "SELECT a.cate, a.cate_name, a.cate_dep, a.cate_sub, a.access_type, a.access_level, SUM(b.cnt) as cnts FROM ( SELECT cate, count(*) as cnt FROM mallRN_goods WHERE display_use = 1 && auth_ck = 'Y' && cate_hide = 0  && vendor_hide = 0  && vendor = '{$vendor}' && SUBSTRING(cate, 1, {$ck_cate_dep}) = '".substr($cate, 0, $ck_cate_dep)."' GROUP BY cate) b JOIN mallRN_cate a ON SUBSTRING(b.cate, 1, {$ck_cate_dep2}) = SUBSTRING(a.cate, 1, {$ck_cate_dep2}) WHERE a.used = '1' && a.cate_parent = '".substr($cate, 0, $ck_cate_dep)."{$ck_cate_dep3}' GROUP BY cate ORDER BY sequence ASC";
	$mysql->query($sql);
	...
	while($row = $mysql->fetch_array()){
		...
		if($row['cate_sub'] == 1) {
			$ck_cate2_dep	= $row['cate_dep'] * 3;
			$ck_cate2_dep2	= $ck_cate2_dep + 3;

			$sql = "SELECT a.cate, a.cate_name, a.cate_dep, a.cate_sub, a.access_type, a.access_level, SUM(b.cnt) as cnts FROM ( SELECT cate, count(*) as cnt FROM mallRN_goods WHERE display_use = 1 && auth_ck = 'Y' && cate_hide = 0  && vendor_hide = 0  && vendor = '{$vendor}' && SUBSTRING(cate, 1, {$ck_cate2_dep}) = '".substr($row['cate'], 0, $ck_cate2_dep)."' GROUP BY cate) b JOIN mallRN_cate a ON SUBSTRING(b.cate, 1, {$ck_cate2_dep2}) = SUBSTRING(a.cate, 1, {$ck_cate2_dep}) WHERE a.cate_parent = '{$row['cate']}' && a.used = '1' GROUP BY cate ORDER BY sequence ASC";
