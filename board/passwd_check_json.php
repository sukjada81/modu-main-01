...
if(!$b_id || !$uid || !$passwd) json_error_msg('필수정보가 제대로 넘어오지 못했습니다.');

if($c_uid) {
	$sql = "SELECT * FROM mallRN_board_{$b_id}_comment WHERE uid = '{$c_uid}'";
	if(!$data = $mysql->one_row($sql)) json_error_msg('등록된 댓글이 없거나 삭제되었습니다.');
	...
	if($my_level < 100) {
		if($data['secret'] == 1 && $data['depth'] > 0) {
			...
			else if($data['o_id'] != $my_id) json_error_msg('회원정보가 일치하지 않습니다.');
		}
		else {
			if(!$data['id']) {
				$passwd = md5($passwd);
