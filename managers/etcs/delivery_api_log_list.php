...
$tpl->define("main","delivery_api_log_list.html");
$tpl->scan_area("main");
...
######################## listPaging 정의 #############################
$listPaging						= new listPaging('mallRN_delivery_api_log'); 
$listPaging->search_variable	= array('status' => 1, 'field' => 0, 'keyword' => 0, 'date_type' => 5, 's_date' => 5 , 'e_date' => 2, 'sort' => 0, 'limit' => 0, 'page' => 0, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'order_num' => '', 'og_uid' => 'function', 'delivery_name' => '', 'delivery_num' => '', 'delivery_code' => '', 'message' => '', 'status' => 'array', 'signdate' => 'datetime');
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array			= array("order_num", "message", "delivery_name", "delivery_num");
$status_array			= array("상태변동없음", "주문완료처리", "연동오류");
######################## list_variable : array일 경우 정의 #############################

######################## list_variable : funtion일 경우 정의 #############################
$og_uid_function = function ($vls) {
	global $mysql;

	$sql	= "SELECT g_name FROM mallRN_order_goods WHERE uid = '{$vls}'";
	return stripslashes($mysql->get_one($sql));
};
######################## list_variable : funtion일 경우 정의 #############################
