...
$sql = "SELECT year FROM mallRN_count_list ORDER BY uid ASC LIMIT 1";
...
for($i = 0; $i < 4; $i ++) {
	$sql					= "SELECT SUM(total) as p_total, SUM(mtotal) as m_total FROM mallRN_count_list WHERE {$where} && type = '{$i}'";
...
for($i = $start; $i <= $end; $i++) {
	...
	for($j = 0; $j < 4; $j ++) {
		if($mode == 'year' || $mode == 'month') {
			$sql = "SELECT SUM(total) as total, SUM(mtotal) as mtotal FROM mallRN_count_list WHERE {$where} {$rwhere2} && type = '{$j}'";
		}
		else $sql = "SELECT * FROM mallRN_count_list WHERE {$where} {$rwhere2} && type = '{$j}'";		
		
		if($data = $mysql->one_row($sql)) {
			if($mode == 'time') {
				$pc_value		= $data['h_'.$i2] ? $data['h_'.$i2] : 0;
				$mobile_value	= $data['mh_'.$i2] ? $data['mh_'.$i2] : 0;
			}
			else {
				$pc_value		= $data['total'] ? $data['total'] : 0;
				$mobile_value	= $data['mtotal'] ? $data['mtotal'] : 0;
			}
