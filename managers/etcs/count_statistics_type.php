...
$sql = "SELECT year FROM mallRN_count_{$type} ORDER BY uid ASC LIMIT 1";
...
if($mode == 'week') {
	...
	if($end_wday < $last_day) {
		$i			= date("d", strtotime("+1 DAY", strtotime($end_wday)));
		$i2			= sprintf("%02d", $i);
		$w			= date("w", strtotime("{$year}-{$month}-{$i2}"));
		$start_wday = date("Y-m-d", strtotime("-".$w." DAY", strtotime("{$year}-{$month}-{$i2}")));
		$end_wday	= date("Y-m-d", strtotime("+6 DAY", strtotime($start_wday)));
...
if($ck_concat == 1) {
	$sql	= "SELECT sum(count) as total FROM ( SELECT  uid, concat(year, lpad(month, 2, 0), lpad(day, 2, 0)) as trdate FROM mallRN_count_{$type} ) b JOIN mallRN_count_{$type} c ON b.uid = c.uid WHERE {$where}";
}
else {
	$sql	= "SELECT SUM(count) as total FROM mallRN_count_{$type} WHERE {$where}";
}
...
if($ck_concat == 1) {
	$sql	= "SELECT c.content, b.count2 as count FROM ( SELECT c1.uid, SUM(count) as count2 FROM ( SELECT uid, concat(year, lpad(month, 2, 0), lpad(day, 2, 0)) as trdate FROM mallRN_count_{$type} ) b1 JOIN mallRN_count_{$type} c1 ON b1.uid = c1.uid WHERE {$where} GROUP BY content ) b JOIN mallRN_count_{$type} c ON b.uid = c.uid";
}
else {
	$sql = "SELECT c.content, b.count2 as count FROM ( SELECT uid, SUM(count) as count2 FROM mallRN_count_{$type} WHERE {$where} GROUP BY content ) b JOIN mallRN_count_{$type} c ON b.uid = c.uid";
}
$mysql->query($sql);
...
if(count($DATAS) > 0) {
	...
	foreach ($DATAS as $key => $row) {
		...
		if($TOTAL == 0) $PER	= $WIDTH = 0;
			...
			if($NUM < 41) $tpl->parse("loop_content");
		}
