...
$tpl->define("main","order_cancel_cp_log_list.html");
$tpl->scan_area("main");
...
######################## listPaging 정의 #############################
$listPaging						= new listPaging('mallRN_order_cancel_cp_log'); 
$listPaging->search_variable	= array('proc' => 1, 'pay_type' => 1, 'status' => 1, 'field' => 0, 'keyword' => 0, 'date_type' => 5, 's_date' => 5 , 'e_date' => 2, 'sort' => 0, 'limit' => 0, 'page' => 0, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'order_num' => '', 'pay_number' => '', 'pay_type' => 'array', 'price' => 'number', 'rem_price' => 'number', 'message' => '', 'status' => 'function|proc', 'signdate' => 'datetime');
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array			= array("order_num", "message", "pay_number");
$status_array			= array("정상처리", "연동오류");
$pay_type_array			= array("B" => "무통장입금", "C" => "카드결제", "R" => "실시간계좌이체", "V" => "가상계좌이체", "H" => "휴대폰결제", "M" => "마일리지");
######################## list_variable : array일 경우 정의 #############################

######################## list_variable : funtion일 경우 정의 #############################
$status_function = function ($vls, $proc) {
	global $tpl, $status_array;
	if($vls == 1 && $proc == 0) {
		$tpl->parse("is_btn_proc");
	}
