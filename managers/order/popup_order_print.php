...
while($info = $mysql->fetch_array()){
	...
	$sql			= "SELECT count(*) FROM mallRN_order_goods WHERE order_num = '{$order_num}' && reals = 1 {$vwhere}";
	...
	$sql = "SELECT * FROM mallRN_order_goods WHERE order_num = '{$order_num}' && reals = 1 {$vwhere} ORDER BY vendor_delivery ASC, status ASC, uid DESC";
	$mysql->query2($sql);
