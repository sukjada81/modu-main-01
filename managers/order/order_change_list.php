...
######################## 사유 정보 #############################
$sql = "SELECT distinct(reason) FROM mallRN_order_status_change";
$mysql->query($sql);
...
$listPaging->search_variable	= array('field' => 0, 'keyword' => 0, 'date_type' => 5, 's_date' => 5, 'e_date' => 2, 'status' => 2, 'reason' => 1, 'vendor' => 1, 'sort' => 0, 'limit' => 0, 'page' => 0, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'order_num' => 'function|name|id|og_uid', 'vendor' => 'vendor', 'message' => '', 'reason' => 'function', 'manager' => '', 'status' => 'function|status2', 'status_date' => 'datetime', 'signdate' => 'datetime');
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array					= array("order_num", "id", "name", "manager");
...
$order_num_function = function ($vls, $name, $id, $og_uid) {
	...
	if($id) {
		...
		if($data['level']) 	$GLOBALS['MEMBER_LEVEL']	= $level_array[$data['level']];
...
$status_function = function ($vls, $status2) {	
	...
	if(($vls == 9 && $status2 == 1) || ($vls == 8 && $status2 == 3)) {
		if($vls == 9)	$GLOBALS['PROC_STATUS'] = "95";
		else			$GLOBALS['PROC_STATUS'] = "85";
		$tpl->parse("is_status_ok");
	}
...
$status_where = function ($vls) {
	if($vls == 1) {
		return "&& a.status2 = 1 && (a.status = 7 || a.status = 8)";
	}
	else if($vls == 2) {
		return "&& ((a.status = 7 && (a.status2 = 2 || a.status2 = 3)) || (a.status = 8 && a.status2 = 2))";
	}
	else if($vls == 3) {
		return "&& ((a.status = 9 && a.status2 = 1) || (a.status = 8 && a.status2 = 3))";
	}
	else {
		$status	= substr($vls, 0, 1);
		$status2 = substr($vls, 1, 1);

		return "&& a.status = '{$status}' && a.status2 = '{$status2}'";
	}	
};
######################## search_variable : 2, 3일 경우 함수 정의 #############################
