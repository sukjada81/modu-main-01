...
$tpl->define("main","headquarters_order_list.html");
$tpl->scan_area("main");
...
$listPaging->list_variable		= array('uid' => '', 'order_num' => 'function|status|name|id', 'g_image' => 'function|g_uid', 'g_name' => '', 'option_name' => 'function', 'qty' => 'number', 'price' => 'function|qty|delivery_type|delivery_type_qty|delivery_price|order_num|delivery_add_price|option|g_uid', 'delivery_info' => 'function|status|status_date', 'status' => 'function|status2', 'status_date' => 'datetime2', 'pay_type' => 'array', 'pay_status' => 'array', 'g_uid' => '', 'signdate' => 'datetime');
$listPaging->field_where		= array('id', 'name', 'cell', 'email', 'name2', 'cell2', 'bank_info');
if(strlen($status)) $listPaging->default_where = "vendor_delivery = '' && status = '{$status}'";
else				$listPaging->default_where = "vendor_delivery = ''";
######################## listPaging 정의 #############################
...
$option_name_function = function ($vls) {	
	global $tpl;

	$GLOBALS['OPTION_NAME']	= $vls;
	$tpl->parse("is_option_name");	
};

$price_function = function ($vls, $qty, $delivery_type, $delivery_type_qty, $delivery_price, $order_num, $delivery_add_price, $option, $g_uid) {	
	global $mysql, $tpl, $DELIVERY_CK_ARRAY;
	...
	if($delivery_price) {
		...
		if(!in_array($order_num, $DELIVERY_CK_ARRAY)) {
			$sql	= "SELECT price FROM mallRN_order_delivery WHERE order_num = '{$order_num}' && vendor = ''";
...
$cqueryPrint = function ($where, $start_record, $page_record_num) {
	...
	return "SELECT c.*, b.name, b.id, b.pay_type, b.pay_status FROM ( SELECT a.order_num, a.name, a.id, a.pay_type, a.pay_status FROM mallRN_order_info a WHERE a.reals = 1 {$where2} ) b JOIN mallRN_order_goods c ON b.order_num = c.order_num {$where} ORDER BY c.{$GLOBALS['sort']} LIMIT {$start_record}, {$page_record_num}";	
};
######################## 독립 쿼리 사용시 함수 정의 #############################
