...
$tpl->define("main","margin_list.html");
$tpl->scan_area("main");
...
$listPaging->list_variable		= array('uid' => 'function|status', 'order_num' => '', 'title' => '', 'price' => 'function|type|commission', 'type' => 'array', 'status' => 'array', 'signdate' => 'datetime');
$listPaging->default_where		= " type != 1";
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array			= array("title", "order_num");
$type_array				= array("상품", "배송비", "마일리지", "쿠폰", "할인", "CP수수료");
$status_array			= array("마진발생", "마진차감");
######################## list_variable : array일 경우 정의 #############################
...
$price_function = function ($vls, $type, $commission) {
	global $tpl;

	if($type == 0)	{
		$GLOBALS['PRICE_INFO1'] = number_format($vls);
		$GLOBALS['PRICE_INFO2'] = number_format($vls - $commission);		

		$tpl->parse("is_price_info");
