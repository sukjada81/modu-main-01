...
$tpl->define("main","margin_statistics.html");
$tpl->scan_area("main");
...
for($i = $i2 = 0; $i < 6; $i ++) {
	if($i == 1) continue;	
	if($i == 0) {
		$sql		= "SELECT SUM(IF(mobile = 'N', IF(status = 0, commission, -commission), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, commission, -commission), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && type = '{$i}'";
	}
	else {
	...
	$total_per[$i2]			= $TOTAL;
	$total_per_pc[$i2]		= $PTOTAL;
	$total_per_mobile[$i2]	= $MTOTAL;
...
$margin_value			= getMarginPer($SALES_PTOTAL, $SALES_MTOTAL, $SALES_TOTAL, "{$where} && type = 0"); 

$SALES_PPER				= $margin_value[0];
$SALES_MPER				= $margin_value[1];
$SALES_PER				= (int) str_replace(",", "", $margin_value[2]) * 100;

$tpl->parse("loop_type_sales_per");
...
for($i = $start; $i <= $end; $i++) {
	...
	$sql = "SELECT SUM(IF(type = 0 && mobile = 'N', IF(status = 0, commission, -commission), 0)) as p_total0, SUM(IF(type = 2 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total2, SUM(IF(type = 3 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total3, SUM(IF(type = 4 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total4, SUM(IF(type = 5 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total5, SUM(IF(type = 0 && mobile = 'Y', IF(status = 0, commission, -commission), 0)) as m_total0, SUM(IF(type = 2 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total2, SUM(IF(type = 3 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total3, SUM(IF(type = 4 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total4, SUM(IF(type = 5 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total5 FROM mallRN_order_sales WHERE {$rwhere2}";		
	$data = $mysql->one_row($sql);
	...
	for($j = $j2 = 0; $j < 6; $j ++) {
		...
		${"COUNT".($j2 + 1)}		= number_format($value);
		${"COUNT_PC".($j2 + 1)}		= number_format($pc_value);
		${"COUNT_MOBILE".($j2 + 1)}	= number_format($mobile_value);

		if($total_per[$j2] != 0)		${"PER".($j2 + 1)}	= number_format((100 * $value) / $total_per[$j2], 2);
		else							${"PER".($j2 + 1)} = 0;

		if($total_per_pc[$j2] != 0)		${"PER_PC".($j2 + 1)}	= number_format((100 * $pc_value) / $total_per_pc[$j2], 2);
		else							${"PER_PC".($j2 + 1)} = 0;

		if($total_per_mobile[$j2] != 0)	${"PER_MOBILE".($j2 + 1)}	= number_format((100 * $mobile_value) / $total_per_mobile[$j2], 2);
		else							${"PER_MOBILE".($j2 + 1)} = 0;

		$COUNTSTR[$j2][]			= $value;
		$COUNTSTRPC[$j2][]		= $pc_value;
		$COUNTSTRMOBILE[$j2][]	= $mobile_value;
	...
	$margin_value				= getMarginPer($SALES_COUNTSTRPC, $SALES_COUNTSTRMOBILE, $SALES_COUNTSTR, "{$rwhere2} && type = 0"); 

	$PMARGIN					= $margin_value[0];
	$MMARGIN					= $margin_value[1];
	...
	$COUNTSTR[5][]				= $SALES_COUNTSTR;
	$COUNTSTRPC[5][]			= $SALES_COUNTSTRPC;
	$COUNTSTRMOBILE[5][]		= $SALES_COUNTSTRMOBILE;
