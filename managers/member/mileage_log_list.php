...
$tpl->define("main","mileage_log_list.html");
$tpl->scan_area("main");
...
######################## listPaging 정의 #############################
$listPaging						= new listPaging('mallRN_mileage_log'); 
...
$listPaging->list_variable		= array('uid' => '', 'id' => 'function', 'content' => '', 'mileage' => 'number', 'use_mileage' => 'number', 'log_proc_id' => '', 'log_proc_acc_ip' => '', 'signdate' => 'datetime', 'status' => 'function');
$listPaging->field_where		= array('id');
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array			= array("id", "log_proc_id", "log_proc_acc_ip");
######################## list_variable : array일 경우 정의 #############################
...
$id_function = function ($vls) {
	...
	$GLOBALS['MEMBER_MILEAGE']	= number_format($data['mileage']);

	return $vls;	
};

$status_function = function ($vls) {
	global $tpl;

	$tpl->parse("is_status{$vls}");
};
######################## list_variable : funtion일 경우 정의 #############################
