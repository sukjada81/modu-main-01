...
$tpl->define("main","popup_goods_search.html");
$tpl->scan_area("main");
...
$type2			= checkGetVar('type2');
$type3			= checkGetVar('type3');
$search_cate	= checkGetVar('search_cate');;
$cate_where		= "";
$ck_search_open = 0;

if($type2 == 2 && $search_cate) {
	$cate_where = " && cate = '{$search_cate}'";
	if(!isset($_GET['cate'])) {
		$_GET['cate'] = $search_cate;
		$ck_search_open = 1;
	}
}
...
######################## 분류 정보 #############################
$sql = "SELECT cate, cate_name FROM mallRN_cate WHERE cate_dep = 1 {$cate_where} ORDER BY sequence ASC";
$mysql->query($sql);
...
$listPaging->search_variable	= array('field' => 0, 'keyword' => 0, 'cate' => 2, 'date_type' => 5, 's_date' => 5, 'e_date' => 2, 'field2' => 0, 'keyword2' => 0, 'field3' => 0, 'keyword3' => 0, 'field4' => 0, 'keyword4' => 0, 'sort' => 0, 'limit' => 0, 'page' => 0, 'display_use' => 1, 'sale_use' => 1, 'option_use' => 1, 'mileage_type' => 1, 'delivery_type' => 1, 'engine_use' => 1, 'order_priority' => 1, 'qty_type' => 1, 'vendor' => 1, 'limit_qty' => 2, 'cate_hide' => 1, 'soldout' => 2, 'option_soldout' => 1, 'commission_type' => 1, 'information_use' => 1, 'type' => 6, 'type2' => 6, 'type3' => 6, 'search_cate' => 6, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'image3' => 'function|moddate', 'name' => '', 'cate' => 'cate', 'price' => 'goods_price', 'display_use' => 'array', 'sale_use' => 'array', 'qty_type' => 'function|qty|uid|option_use|name');
######################## listPaging 정의 #############################
...
$qty_type_function = function ($vls, $qty, $uid, $option_use, $gname) {	
	...
	if($option_use==1) {
		...
		$return = "<span class='optionView size09' data-uid='{$uid}' gname='{$gname}'>옵션</span><br />";
...
$ck_search_open2 = 0;
if($type == 'display' && $type2 && $type3) {
	if(!$listPaging->where) $ck_search_open2 = 1;
	$listPaging->default_where = "main{$type2}_display{$type3} = '0'";
	$listPaging->where = $listPaging->where." && a.main{$type2}_display{$type3} = '0'";
}

if($type == 'exhibition' && $type2) {
	if(!$listPaging->where) $ck_search_open2 = 1;
	$listPaging->default_where = "!INSTR(exhibition, ',{$type2},')";
	$listPaging->where = $listPaging->where." && !INSTR(a.exhibition, ',{$type2},')";	
}
...
if($listPaging->check_where == 1) {	
	...
	$tpl->parse("is_search");
	if($ck_search_open2 == 0) {
		if($ck_search_open == 0) $tpl->parse("is_search_open1");
		$tpl->parse("is_search_open2");
	}
}
######################## 검색 조건 있는 경우 #############################	
...
if($type=='relatedGoods' || $type=='display'  || $type=='exhibition') {
	$multi = 1;
	$disabled = "";
}
else {
