...
if($type == 'sales') {
	######################## 판매금액 #############################
	$sql = "SELECT SUM(IF(mobile = 'N', IF(status = 0, price, -price), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total, SUM(IF(status = 0, price, -price)) as sum, g_uid FROM mallRN_order_sales WHERE {$where} && type = 0 GROUP BY g_uid ORDER BY sum DESC LIMIT 30";
	######################## 판매금액 #############################
}
else if($type == 'qtys') {
	######################## 판매수량 #############################
