...
$tpl->define("main","popup_list.html");
$tpl->scan_area("main");
...
######################## listPaging 정의 #############################
$listPaging						= new listPaging('mallRN_popup'); 
$listPaging->search_variable	= array('field' => 0, 'keyword' => 0, 'type' => 1, 'status' => 1, 'period' => 2, 'sort' => 0, 'limit' => 0, 'page' => 0, 'atotal_record' => 0, 'total_record' => 0);
$listPaging->list_variable		= array('uid' => '', 'name' => '', 'period' => 'function|s_date|e_date', 'type' => 'array', 'status'=>'array', 'position'=>'array', 'signdate'=>'date');
$listPaging->field_where		= array('goods_name');
######################## listPaging 정의 #############################

######################## list_variable : array일 경우 정의 #############################
$multi_array					= "";
$status_array					= array("0" => "사용함", "1" => "사용안함", "2" => "기간만료"); 
$type_array						= array("0" => "항상", "1" => "하루에 한번"); 
$position_array					= array("0" => "직접입력", "1" => "상단좌측", "2" => "상단중앙", "3" => "상단우측", "4" => "중앙좌측", "5" => "정중앙", "6" => "중단우측", "7" => "하단좌측", "8" => "하단중앙", "9" => "하단우측"); 
######################## list_variable : array일 경우 정의 #############################

######################## list_variable : funtion일 경우 정의 #############################
$period_function = function ($vls, $s_date, $e_date) {	
	if($vls == 0) return "무기한";
	else return $s_date.' ~ '.$e_date;
};
######################## list_variable : funtion일 경우 정의 #############################
