...
for($i = 0; $i < 2; $i ++) {
	$sql					= "SELECT SUM(IF(mobile = 'N', IF(status = 0, price, -price), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && type = '{$i}' {$where2}";
...
for($i = $start; $i <= $end; $i++) {
	...
	$sql = "SELECT SUM(IF(type = 0 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total0, SUM(IF(type = 1 && mobile = 'N', IF(status = 0, price, -price), 0)) as p_total1,  SUM(IF(type = 0 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total0, SUM(IF(type = 1 && mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total1 FROM mallRN_order_sales WHERE {$rwhere2} {$where2}";	
	$data = $mysql->one_row($sql);
	...
	$COUNTSTR[2][]				= $SALES_COUNTSTR;
	$COUNTSTRPC[2][]			= $SALES_COUNTSTRPC;
	$COUNTSTRMOBILE[2][]		= $SALES_COUNTSTRMOBILE;
