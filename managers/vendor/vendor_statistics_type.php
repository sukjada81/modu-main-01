...
$tpl->define("main","vendor_statistics_type.html");
$tpl->scan_area("main");
...
if($type != 'favorite' && $type != 'commission') $type = 'sales'; 

switch($type) {
	case "sales" : 			$TTL = "판매금액"; break;	
	case "commission" : 	$TTL = "판매수수료"; break;	
...
while($row = $mysql->fetch_array()){
	...
	$vendor_array[$vendor_id]	= $vendor_name."({$vendor_id})";	
}
...
if($type == 'sales') {
	######################## 매출 #############################
	$sql = "SELECT DISTINCT(vendor) FROM mallRN_order_sales WHERE {$where} && type = 0 && vendor != ''";
	$mysql->query($sql);

	while($row = $mysql->fetch_array()) {		
		
		$value	= $p_total = $m_total = 0;
		$sql	= "SELECT SUM(IF(mobile = 'N', IF(status = 0, price, -price), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && vendor = '{$row['vendor']}'";
		...
		$DATAS[]		= array("sum" => $value, "title" => $vendor_array[$row['vendor']], "vendor" => $row['vendor']);
		$DATAS_PC[]		= array("sum" => $p_total, "title" => $vendor_array[$row['vendor']], "vendor" => $row['vendor']);
		$DATAS_MOBILE[] = array("sum" => $m_total, "title" => $vendor_array[$row['vendor']], "vendor" => $row['vendor']);
	...
	while($row = $mysql->fetch_array()) {		
		...
		$value	= $p_total = $m_total = 0;
		$sql	= "SELECT SUM(IF(mobile = 'N', IF(status = 0, commission, -commission), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, commission, -commission), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && vendor = '{$row['vendor']}' && type = 0";
	...
	######################## 관심상품 #############################
	$sql = "SELECT SUM(IF(mobile = 'N', 1, 0)) as p_total, SUM(IF(mobile = 'Y', 1, 0)) as m_total, COUNT(uid) as sum, vendor FROM mallRN_favorite_store WHERE {$where} GROUP BY vendor ORDER BY sum DESC LIMIT 30";	
