...
while($row = $mysql->fetch_array()){
	...
	$tpl->parse("loop_vendor1");
	$tpl->parse("loop_vendor2");
}
...
if($vendor) $where2 = "&& vendor = '{$vendor}'";
else		$where2	= "&& vendor != ''";
...
for($i = 0; $i < 3; $i ++) {
	if($i < 2) {
		$sql					= "SELECT SUM(IF(mobile = 'N', IF(status = 0, price, -price), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, price, -price), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && type = '{$i}' && confirmation = 1 {$where2}";
	}
	else {
		$sql					= "SELECT SUM(IF(mobile = 'N', IF(status = 0, commission, -commission), 0)) as p_total, SUM(IF(mobile = 'Y', IF(status = 0, commission, -commission), 0)) as m_total FROM mallRN_order_sales WHERE {$where} && type = '0' && confirmation = 1 {$where2}";
	}
